<?php
    ...
    if(isset($_POST['exporter'])){
        ...
        if(mysqli_num_rows($ch_q) > 0){
            ...
            //Insert Query
            $insert_query = mysqli_query($db, "insert into expoter (exporter_code, exporter_name, gstin, license_name, license_number, email, contact, website) values ('$exporter_code', '$exporter_name', '$exporter_gstin_number', '$exporter_license_name', '$exporter_license_number', '$exporter_email', '$exporter_contact','$exporter_website')");
            if(!$insert_query){
                echo "<script>alert('Something Went Wrong .');window.location.href='./';</script>";
            }
            else{
                $insert_sub_info = mysqli_query($db, "insert into exporter_sub_info (exporter_code, address, country, registration_date, source_country, delicensed_date, re_registration_date) values('$exporter_code', '$exporter_add', '$exporter_country', '$registered_date', '$source_country', '$exporter_delicensed_date', '$exporter_re_registration_date')");
    ...
    if(isset($_POST['importer'])){
        ...
        $importer_email = mysqli_real_escape_string($db, $_POST['email']);
        $importer_contact = mysqli_real_escape_string($db, $_POST['contact']);
        ...
        if(mysqli_num_rows($ch_q) > 0){
            ...
            //Insert Query
            $insert_query = mysqli_query($db, "insert into importer (importer_code, importer_name, gstin, license_name, license_number, email, contact, website) values ('$importer_code', '$importer_name', '$importer_gstin_number', '$importer_license_name', '$importer_license_number', '$importer_email', '$importer_contact', '$importer_website')");
            if(!$insert_query){
                echo "<script>alert('Something Went Wrong .');window.location.href='./';</script>";
            }
            else{
                $insert_sub_info = mysqli_query($db, "insert into importer_sub_info (importer_code, address, country, registration_date, source_country, delicensed_date, re_registration_date) values('$importer_code', '$importer_add', '$importer_country', '$registered_date', '$source_country', '$importer_delicensed_date', '$importer_re_registration_date')");
